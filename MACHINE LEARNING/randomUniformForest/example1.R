...
data(carEvaluation)
car.data <- carEvaluation %T>% print

n <- nrow(car.data) %T>% print
p <- ncol(car.data) %T>% print

trainTestIdx <- cut(sample(1:n, n), 2, labels= FALSE) %T>% print

# train examples
car.data.train <- car.data[trainTestIdx == 1, -p] %T>% print
car.class.train <- as.factor(car.data[trainTestIdx == 1, p]) %T>% print

# test data
car.data.test <- car.data[trainTestIdx == 2, -p] %T>% print
car.class.test <- as.factor(car.data[trainTestIdx == 2, p]) %T>% print

# compute model : train then test in the same function
car.ruf <- randomUniformForest(car.data.train, car.class.train,
xtest = car.data.test, ytest = car.class.test, threads = 2) %T>% print

# compute importance
car.ruf.importance <- importance.randomUniformForest(car.ruf, Xtest = car.data.train,
                                                     threads = 2) %T>% print
